...
def 当星反应(周号,名): # 文件均含开/高/收/低四个价格,停牌时股价可能为0,这时换成nan防画入||益矩:[因子数,点数,天数],方矩则存储对应例数 # 48-6160 # 88-8927 # 57-6668 # 72-8003
    起=3; f=pp(l:=pd.read_csv(名,encoding=['utf-8','gbk'][0]).replace(0,np.nan).iloc[:,:600]); print(l.shape); 掩=pp([False]*(f.shape[1]-起)); 掩[-[1000,2400][名=='今全价.csv']:]=True; df=f[:,起:][:,掩].copy() 
    ...
    for 股号 in tqdm(r(0,min(len(f)//价数,20000)),position=0,leave=True): # 0.开盘买次日卖|1.收盘买次日卖|2.开盘买当日收盘卖(买卖点其一也可改成中盘)||涨表和左涨表是收/开盘买的自变量 
        ...
        开价涨=np.where(开挂<o,开开涨,np.where(高挂<h,开当涨+高挂,开价涨)); 价价涨=np.where(开挂<o,o,np.where(高挂<h,高挂,价价涨)); 左价涨,左高涨,左低涨,左开涨,_=左涨表; 左前涨=左(前涨); 左左涨=左(左前涨) 
        ...
        for 点 in 掩点((pp(周集)*周号==周号*周号)*去端): # 点即买点,三条件:1.当日收盘未涨停而可买入|2.当日属特定星期|3.排除无参考和无验证的端部||下面对益限幅以过滤因停牌产生的跳空点  
            if 买点==3 and len(价线)-点>23 and abs(价涨[点])<11 and abs(前涨[点])<22 and abs(左前涨[点])<22 and 高涨[点]>-19 and rd(价涨[点])==0 and rd(前涨[点])==0 and rd(左前涨[点])==0: 置矩[i:=int(左左涨[点])+10,:,点]+=pp(邻变(价线[点:点+22]));  数矩[i,:,点]+=1; continue 
            ...
            if 买点==1 and 价涨[点]<9 and all([abs(x[点])<10.4 for x in 涨表]) and abs(中涨[点])<10.4: # 1.收盘买入,次日开/中/收卖出(标况要求收盘价贴合最低价,前涨正负3高涨正3,前消则微逊) 
                ...
                for i in r(len(益矩)): 益矩[i,j:=rd(涨表[i][点])+宽,点]+=价价涨[点]; 方矩[i,j,点]+=1 # 据卖点选: 价开涨/价价涨/价中涨  
    ...
    if 宽矩.max()==0: 益矩,方矩=解构(益矩,方矩); x=plt.subplots(2,1,figsize=(6,4))[1]; 画图(x[0],方矩); 画图(x[1],益矩); plt.show() 
...
串否=1; a=np.minimum.reduce([当星反应(5,名) for 名 in 集]); x=plt.subplots(1,1,figsize=(6,[2,5][回热:=len(a)>6]))[1]; 热图(x,a) if 回热 else 画图(x,a); plt.show() 
